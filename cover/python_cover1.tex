...
% xelatex -output-driver="xdvipdfmx" python_cover1.tex
%
...
\tikzset{
  hilbert/.style = {bottom color=black, middle color = white, top color=white, shading angle=0, draw=black, opacity=.14},
...
\begin{document}%
  \begin{tikzpicture}
    ...
    % Background
    % ---------------------------------------------------
	\begin{scope}[shift={(Cover.south west)},xshift=-37cm,yshift=-0.4cm,scale=4]
	%\begin{scope}[shift={(Cover.south west)},xshift=-17cm,yshift=-0.6cm,scale=3]     
	\shadedraw[hilbert]
	[l-system={Sierpinski arrow, axiom=X,  order=7, angle=-60}] %step={\CropW/31+.01pt},
	lindenmayer system; 
	\end{scope}      
    % ---------------------------------------------------
    ...
    \begin{scope}[scale=0.7,transform canvas ={yshift=21mm}]
      ...
      \path (Cover.east) +(55mm,-145mm) node[left,scale=1.8,text width=.20\PageW,align=left,fill=white,text=subtitlealgo,inner sep=.4em,font=\authorfont,rounded corners=10pt]{\bf ~~tome 2~~\vphantom{.}};
    ...
    \begin{scope}[transform canvas ={yshift=21mm}]
      \node[rotate=90,scale=1.8] (tranche) at (Spine.center)
        {\textcolor{spinepython}{PYTHON}\textcolor{spineau}{AU}\textcolor{spinelycee}{LYCÉE}\textcolor{spinealgo}{~~~\textemdash~~tome 2}\vphantom{\large y}};
    \end{scope}
    % ---------------------------------------------------
    %  Logos Exo7
    % ---------------------------------------------------
...
Ce second tome est parfait pour approfondir votre connaissance de \emph{Python}, des algorithmes et de la programmation.
...
\item Nombres complexes I \& II
\item Dérivée -- Zéros de fonctions
\item Exponentielle
\item Logarithme
\item Intégrale
\item Programmation objet
\item Mouvement de particules
\item Algorithmes récursifs
\item Tri -- Complexité
\item Calculs en parallèle
\end{itemize}
\end{minipage}
\begin{minipage}{0.49\textwidth}
\begin{itemize}
...
\item Le mot le plus long
\item Images et matrices
\item Ensemble de Mandelbrot
\item Images 3D 
\item Sudoku
\item Fractale de Lyapunov
\item Big data I \& II
\end{itemize}
\end{minipage}
\end{center}    
