...
def tirage_total(Nmin=100,Nmax=999):
    """ Tirage aléatoire d'un total à atteindre """
...
def tirage_chiffres(nb_chiffres=6):
    """ Tirage aléatoire sans remise d'un certains nb de chiffres """
...
def operation(a,b,op):
    """ Résultat du calcul étant donné deux nb et une opération """
    if op == '+':
        calcul = a + b
    elif op == '-':
        calcul = a - b
...
def recherche_basique(total,chiffres):
  """ Recherche des solutions séquentielles (recherche pas exhaustive) """  
...
def atteindre_somme(S,chiffres):
    """ Comment atteindre la somme S à l'aide des chiffres donnés """
    # Cas terminaux
    if S == 0:
        return []
    if S < 0:
...
def compte_est_bon(total,chiffres):
    """ Renvoie solution du compte est bon
        Entrée : le total à atteindre et une liste de chiffres
        Sortie : une suite d'opération permettant d'atteindre le total
